...
# Integration points:
# - Uses MariaDB/MySQL database with DBI interface for data persistence
...
sub new {
    ...
    # Load database credentials from environment variables for Docker compatibility
    if ($ENV{DB_USER} && $ENV{DB_PASS}) {
        $authDbUser = $ENV{DB_USER};                         # Database username from environment
        $authDbPass = $ENV{DB_PASS};                         # Database password from environment
        $db_host = $ENV{DB_HOST} || 'localhost';             # Database host with localhost default
    ...
    my $db_port = $ENV{DB_PORT} || '3306';                  # Port with MySQL default
    
    # Construct DSN based on connection parameters for optimal performance
    my $dsn;
    if ($db_host eq 'localhost' && $db_port eq '3306') {
        # Use simplified DSN for localhost default port
        $dsn = "DBI:MariaDB:$db_name:$db_host";
    } elsif ($db_host eq 'localhost') {
        # Use full specification for localhost non-default port
        $dsn = "DBI:MariaDB:database=$db_name;host=$db_host;port=$db_port";
    } else {
        # Use full specification for remote database connections
        $dsn = "DBI:MariaDB:database=$db_name;host=$db_host;port=$db_port";
    }
...
sub save_unified_stashes {
    ...
    my $sth = $self->{dbh}->prepare(
        "INSERT INTO stashes (user_id, stash_data) VALUES (?, ?) 
...
sub create_password_reset_token {
    ...
    my $sth = $self->{dbh}->prepare(
        "INSERT INTO password_reset_tokens (user_id, token, expires_at) VALUES (?, ?, FROM_UNIXTIME(?))"
    );
...
sub use_reset_token {
    my ($self, $token) = @_;
    $self->ensure_connection;
    my $sth = $self->{dbh}->prepare("UPDATE password_reset_tokens SET used = TRUE WHERE token = ?");
...
sub count_recent_reset_requests {
    ...
    my $sth = $self->{dbh}->prepare(
        "SELECT COUNT(*) FROM password_reset_tokens prt 
         JOIN users u ON prt.user_id = u.id 
         WHERE u.email = ? AND prt.created_at >= DATE_SUB(NOW(), INTERVAL ? MINUTE)"
    );
